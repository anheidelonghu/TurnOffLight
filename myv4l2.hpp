...
class V4L2Capture {
public:
	V4L2Capture(char *devName, int width, int height, __s32 exposure_level);
	virtual ~V4L2Capture();
...
V4L2Capture::V4L2Capture(char *devName, int width, int height, __s32 exposure_level) {
...
int V4L2Capture::initDevice() {
	...
	struct v4l2_control ctrl;		//set exposure
	...
	if (ret < 0) {
		perror("Get exposure auto type error");
		//exit(-1);
	}
	printf("\nGet Exposure Auto Type:[%d]\n", ctrl.value);
	ctrl.value = V4L2_EXPOSURE_MANUAL;  //手动曝光模式 V4L2_EXPOSURE_MANUAL
	ret = ioctl(fd_cam, VIDIOC_S_CTRL, &ctrl);
	if (ret < 0) {
		perror("Set exposure auto type error");
		//exit(-1);
	}
	usleep(10);
	ctrl.id = V4L2_CID_EXPOSURE_ABSOLUTE;
	ctrl.value = exp_lvl;//1=0.1ms
	//cout << typeid(ctrl.value).name() << endl;
	ret = ioctl(fd_cam, VIDIOC_S_CTRL, &ctrl);
	if (ret < 0) {
		perror("Set exposure value error");
		//exit(-1);
	}
...
int V4L2Capture::getFrame(void **frame_buf, size_t* len) {
	...
	*len = buffers[queue_buf.index].length;
	//cout << buffers[queue_buf.index].length << endl;
...
void VideoPlayer() {
	...
	string videoDev = "/dev/v4l/by-path/platform-3f980000.usb-usb-0:1.2:1.0-video-index0";
